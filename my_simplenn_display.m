function [info, str] = my_simplenn_display(net, varargin)
...
for l = 1:numel(net.layers)
  ...
  switch ly.type
    ...
    case 'ccc'
      ks = size(ly.weights{1});
      info.support(1:2,l) = [ks(1) ks(2)] ;
    otherwise
...
for l = 1:numel(net.layers)
  ...
  switch ly.type
    ...
    case 'ccc'
      info.dataSize(1,l+1) = numel(ly.marray) ;
      info.dataSize(2,l+1) = info.dataSize(2,l) ;
      info.dataSize(3,l+1) = factorial(info.dataSize(3)) / (2 * factorial(info.dataSize(3) - 2));
    case 'custom'
            ...
            case 'nfilt'
              switch ly.type
                case 'conv'
                  ...
                  s=sprintf('%d',a) ;
                case 'ccc'
                  if isfield(ly, 'weights'), a = size(ly.weights{1},4) ; end
                  s=sprintf('%d',a) ;
                otherwise
                  s='n/a' ;
              end
